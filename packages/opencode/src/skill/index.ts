...
import { CustomizeOpencodeContent, materializeMattPocockSkills } from "@opencode-ai/core/plugin/skill"
...
const CUSTOMIZE_OPENCODE_SKILL_BODY = CustomizeOpencodeContent
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        const builtInRoot = yield* materializeMattPocockSkills(fsys, global).pipe(Effect.orDie)
        const builtIns: ScanState = { matches: new Set(), dirs: new Set() }
        yield* scan(builtIns, builtInRoot, SKILL_PATTERN, { dot: true, scope: "built-in" })
        yield* loadSkills(s, { matches: Array.from(builtIns.matches), dirs: Array.from(builtIns.dirs) }, events)

...
const projectSkills = (skills: Skill.Info[]) =>
  skills.filter((skill) => skill.location !== "<built-in>" && !skill.location.includes("built-in-skills"))
...
`,
          ...
          const list = projectSkills(yield* skill.all())
...
`,
          ...
          const skill = yield* Skill.Service
          expect(projectSkills(yield* skill.all())).toEqual([])
        }),
      { git: true },
    ),
  )
